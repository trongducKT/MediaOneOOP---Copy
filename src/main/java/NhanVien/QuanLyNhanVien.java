...
public class QuanLyNhanVien {
    ...
    public boolean changeNVCD(String name, String vitri, String hesoluong) {
        for (NhanVien obj : danhsachNhanViens) {
            if (obj.getTenNhanVien().equals(name)) {
                if (obj instanceof NhanVienCoDinh) {
                    NhanVienCoDinh nvcd = (NhanVienCoDinh) obj;
                    if(!vitri.equals("")){
                       nvcd.setViTri(vitri);
                    }
                    if(!hesoluong.equals("")){
                       nvcd.setHeSoLuong((double)Double.parseDouble(hesoluong));
                    }
                    IO.ghiNV(danhsachNhanViens);
                    return true;
                }
